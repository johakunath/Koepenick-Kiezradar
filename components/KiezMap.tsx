    ...
    >
      ...
      {mapped.map((entry) => (
        <Marker key={entry.id} position={[entry.lat!, entry.lng!]}>
          <Popup maxWidth={280}>
            <div style={{ fontFamily: "system-ui, sans-serif", fontSize: 13, lineHeight: 1.4 }}>
              <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em", color: "#2d6080", fontWeight: 600, marginBottom: 4 }}>
                {entry.tags.map((t) => TAG_LABELS[t]).join(" · ")}
              </div>
              <strong style={{ fontSize: 14, lineHeight: 1.3, display: "block", marginBottom: 5, color: "#143d56" }}>
                {entry.title}
              </strong>
              <p style={{ color: "#445562", margin: "0 0 8px", lineHeight: 1.45, fontSize: 12 }}>
                {entry.ai_summary}
              </p>
              <div style={{ display: "flex", gap: 10, alignItems: "center", borderTop: "1px solid #ddd0b8", paddingTop: 6, marginTop: 4 }}>
                <a
